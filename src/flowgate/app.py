...
from flowgate.db.models import CallerToken, ProviderKey, ProviderModel, VaultMeta
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    app = FastAPI(
        title="FlowGate",
        description="Local-first LLM gateway with cost audit dashboard",
        version=__version__,
        lifespan=lifespan,
        docs_url="/docs",
        redoc_url=None,
...
def _try_auto_unlock(vault: Vault, settings: Settings) -> None:
    """If master password is in FLOWGATE_MASTER_PASSWORD env var, auto-unlock."""
    import os
