...
def mf_cache_root_dir() -> str:
    """Project-local HF cache root for bundled MF assets."""
    return os.path.join(os.getcwd(), "models", "huggingface", "hub")


def mf_cache_repo_dir() -> str:
    """Expected bundled directory for MF repo assets."""
    return os.path.join(mf_cache_root_dir(), "models--routellm--mf_gpt4_augmented")


def mf_bundle_dir() -> str:
    """Project-distributed MF bundle directory (tracked in repo)."""
    return os.path.join(os.getcwd(), "models", "huggingface", "bundles", "mf_gpt4_augmented")


def _mf_bundle_model_path() -> str:
    return os.path.join(mf_bundle_dir(), "model.safetensors")


def _mf_legacy_model_path() -> str:
    return os.path.join(mf_cache_repo_dir(), "model.safetensors")


def _ensure_mf_bundle_from_legacy_cache() -> bool:
    """Migrate model from old cache layout to bundle layout if needed."""
    bundle_model = _mf_bundle_model_path()
    if os.path.isfile(bundle_model):
        return True
    legacy_model = _mf_legacy_model_path()
    if not os.path.isfile(legacy_model):
        return False
    os.makedirs(mf_bundle_dir(), exist_ok=True)
    shutil.copy2(legacy_model, bundle_model)
    logger.info("Migrated MF model into bundle directory: %s", bundle_model)
    return True


def mf_assets_cached() -> bool:
    """Whether project has an MF model file ready for local loading."""
    if os.path.isfile(_mf_bundle_model_path()):
        return True
    return _ensure_mf_bundle_from_legacy_cache()
...
class SmartRouterService:
    ...
    def _build(self) -> None:
        ...
        if cfg.strategy == "complexity":
            ...
            # MF requires local bundled assets under project directory.
            if cfg.classifier_type == "mf" and not mf_assets_cached():
                logger.warning(
                    "RouteLLM MF assets are missing in project bundle (%s) — "
                    "falling back to complexity strategy for this runtime.",
                    mf_bundle_dir(),
                )
                self._config.strategy = "complexity"
                self._scorer = ComplexityScorer(
            ...
            try:
                ...
                cfg_api_key = (cfg.mf_embedding_api_key or "").strip()
                if cfg_api_key:
                    openai_kwargs["api_key"] = cfg_api_key
                else:
                    env_api_key = (os.environ.get("OPENAI_API_KEY") or "").strip()
                    if env_api_key:
                        openai_kwargs["api_key"] = env_api_key
                    else:
                        # Supply a dummy key so OpenAI() initialization won't fail.
                        # It only matters for actual MF embedding calls, not BERT.
                        dummy_api_key = "dummy-key-for-routellm-init"
                        os.environ["OPENAI_API_KEY"] = dummy_api_key
                        openai_kwargs["api_key"] = dummy_api_key
                if cfg.mf_embedding_base_url:
                    openai_kwargs["base_url"] = cfg.mf_embedding_base_url

                # routellm initializes OpenAI() during module import, so we must
                # make sure the env var is present before importing routellm modules.
                os.environ["OPENAI_API_KEY"] = openai_kwargs["api_key"]
                ...
                controller_config: dict[str, dict[str, str]] | None = None
                if cfg.classifier_type == "mf":
                    # Use project-local bundle path directly so startup is fully offline.
                    controller_config = {"mf": {"checkpoint_path": mf_bundle_dir()}}
