...
from flow_llm_router import __version__

app = typer.Typer(
    name="flow-router",
    help="Flow LLM Router - Local-first LLM gateway with token usage analytics",
...
def start(
    ...
    config: Optional[str] = typer.Option(None, help="Path to flow_llm_router.yaml config file"),
    reload: bool = typer.Option(False, help="Enable auto-reload for development"),
):
    """Start the Flow LLM Router local gateway."""
    import os

    import uvicorn

    from flow_llm_router.config import load_settings
    ...
    if settings.security.vault_enabled and not os.environ.get("FLOWGATE_MASTER_PASSWORD"):
        from flow_llm_router.security.master_key_store import has_master_key
    ...
    console.print(
        Panel.fit(
            f"[bold cyan]Flow LLM Router[/bold cyan] v{__version__}\n\n"
    ...
    uvicorn.run(
        "flow_llm_router.app:create_app",
        host=actual_host,
        port=actual_port,
        reload=reload,
...
@app.command()
def version():
    """Show Flow LLM Router version."""
    console.print(f"Flow LLM Router v{__version__}")
...
def _prompt_master_password(settings) -> None:
    ...
    from flow_llm_router.db.engine import get_session, init_db
    from flow_llm_router.db.models import VaultMeta
